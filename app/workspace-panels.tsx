...
import { type Pupil, type PresenceStatus } from './presence-data';
...
export function PresencePanel({
  ...
  school: string;
  /** Lifted so the 3D site map and this register read the same records. */
  pupils: Pupil[];
  onChange: (next: Pupil[]) => void;
}) {
  const setPupils = (update: Pupil[] | ((prev: Pupil[]) => Pupil[])) =>
      onChange(typeof update === 'function' ? update(pupils) : update);
  const [query, setQuery] = useState(''),
